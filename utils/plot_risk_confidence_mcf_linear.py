...
def fit_func(x, k, b):  # The variable must be provided as the first argument
    return k * x + b
...
def curve_fit_boundary(label_en, confidence_score_dict, boundary_x_coordinate):
    ...
    boundary_y_coordinate_fit = fit_func(boundary_x_coordinate, const.popt[label_en][0], const.popt[label_en][1])
...
def prepare_predict_information_dict(label_en, pred_pro_weighted_sum, pred_pro_confidence_score, test_data_label_arr, pred_label_arr):
    judge_y = pred_pro_confidence_score - fit_func(pred_pro_weighted_sum, const.popt[label_en][0], const.popt[label_en][1])
...
def print_assessed_values(label_en, label_ch):
    ...
    print('%s fitted threshold curve: y = %.3f x + %.3f\n' % (label_ch, const.popt[label_en][0], const.popt[label_en][1]))
